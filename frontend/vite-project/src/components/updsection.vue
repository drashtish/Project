<template>
  <div>
    ...
    <form class="form" id="signUpForm" @submit.prevent="entry">
      <div class="mb-3">
        <label>Title</label>
        <input type="text" v-model="title" class="form-control" id="signupName">
      </div>
      <div class="mb-3">
        <label for="signupEmail" class="form-label">Date</label>
        <input type="date" v-model="date" class="form-control" id="signupEmail">
      </div>
      <div class="mb-3">
        <label for="signupPassword" class="form-label">Description</label>
        <input type="text" v-model="desc" class="form-control" id="signupPassword">
      </div>
...
export default {
  ...
  methods: {
    ...
    async entry() {
      try {
        const response = await fetch(`http://localhost:5000/updsection/${this.id}`, {
          method: 'POST',
          headers: {
    ...
    async getsec(id) {
      try {
        const response = await fetch(`http://localhost:5000/updsection/${id}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Access-Control-Allow-Origin': '*',
        ...
        const data = await response.json();
        this.d = data.d; // Corrected assignment
        this.title = data.d.title;
        this.date = data.d.date;
        this.desc = data.d.desc;
      } catch (error) {
        alert(error.message);
      }
    }
  ...
  beforeMount() {
    ...
    if (this.token) {
      if (this.role !== 'admin') {
        ...
        this.getsec(this.$route.params.id);
      }
    } else {
      this.$router.push('/adminlogin');
