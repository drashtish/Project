<template>
  <div>
    ...
    <form class="form" id="signUpForm" @submit.prevent="entry">
      ...
      <br>
      <div>
        Upload File(Only if you want to change)<br>
...
export default {
  ...
  methods: {
    ...
    async entry() {
      ...
      try {
        const response = await fetch(`http://localhost:5000/updbook/${this.id}`, {
          method: 'POST',
          body: bookDetails,
          headers: {
    ...
    async getbook(id) {
      try {
        const response = await fetch(`http://localhost:5000/updbook/${id}`, {
          method: 'GET',
          headers: {
